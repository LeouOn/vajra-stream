...
class ComposerService:
    ...
    def compose_healing_music(
        ...
        try:
            composition = self.composer.compose(
                intention=intention, duration=duration_seconds, base_freq=base_frequency, style=style
            )
            return {
                "status": "success",
                "intention": intention,
    ...
    def generate_frequency_sequence(
        self, frequencies: list[float], duration_per_frequency: float = 60.0
    ) -> dict[str, Any]:
        ...
        try:
            sequence = self.composer.create_sequence(frequencies=frequencies, duration_each=duration_per_frequency)
            return {
                "status": "success",
    ...
    def create_solfeggio_progression(self, include_frequencies: list[int] | None = None) -> dict[str, Any]:
    ...
    def compose_meditation_soundscape(
        self, meditation_type: str = "mindfulness", duration_minutes: int = 20
    ) -> dict[str, Any]:
    ...
    def get_available_styles(self) -> list[str]:
        ...
        return ["ambient", "drone", "rhythmic", "melodic", "binaural", "isochronic", "nature_sounds"]

    def get_status(self) -> dict[str, Any]:
        """Get composer status"""
        return {"composer": self.composer is not None, "available_styles": self.get_available_styles()}
