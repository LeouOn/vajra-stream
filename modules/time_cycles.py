...
class TimeCyclesService:
    ...
    def broadcast_to_time_cycle(
        self, target_date: datetime, intention: str = "healing", duration_minutes: int = 30
    ) -> dict[str, Any]:
        ...
        try:
            session = self.broadcaster.broadcast(
                target_date=target_date, intention=intention, duration_minutes=duration_minutes
            )
            return {
                "status": "success",
    ...
    def heal_past_event(
        self, event_date: datetime, event_name: str, intention: str = "healing and liberation"
    ) -> dict[str, Any]:
    ...
    def heal_future_event(
        self, event_date: datetime, event_name: str, intention: str = "protection and blessing"
    ) -> dict[str, Any]:
        """Send blessing/protection to a future event"""
        return self.broadcast_to_time_cycle(
            target_date=event_date, intention=f"Blessing for {event_name}: {intention}", duration_minutes=30
        )

    def continuous_healing_cycle(
        self, start_date: datetime, end_date: datetime, intention: str = "ongoing healing"
    ) -> dict[str, Any]:
        ...
        try:
            cycle = self.broadcaster.continuous_cycle(start_date=start_date, end_date=end_date, intention=intention)
            return {
                "status": "success",
    ...
    def get_current_cycle_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "current_time": datetime.now().isoformat(), "cycle_info": info}
        except Exception as e:
            return {"error": str(e)}

    def get_status(self) -> dict[str, Any]:
        """Get status of time cycle broadcaster"""
        return {"broadcaster": self.broadcaster is not None, "current_time": datetime.now().isoformat()}
