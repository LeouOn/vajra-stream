...
class AudioService:
    ...
    async def generate_audio(self, config: dict) -> dict[str, Any]:
        ...
        try:
            ...
            self.active_frequencies = (
                [frequency] if isinstance(frequency, (int, float)) else config.get("frequencies", [frequency])
            )
