...
class OrchestratorService:
    ...
    async def _trigger_practices_for_hour(self, planet: str, lat: float, lon: float):
        ...
        for practice in self.practices:
            if not practice.preferred_planetary_hours or planet.lower() in [p.lower() for p in practice.preferred_planetary_hours]:
                logger.info(f"Auspicious match! Practice '{practice.name}' aligns with {planet} hour.")
                
                # Find available characters
                available_chars = [c for c in self.char_manager.get_active_characters() if c.state == "Idle" and c.energy >= 50]
