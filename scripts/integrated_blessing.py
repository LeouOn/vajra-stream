...
class IntegratedBlessing:
    ...
    def start_blessing_session(self, intention="peace", duration=60):
        ...
        visual_thread = threading.Thread(target=self._run_visual, args=(intention, duration), daemon=True)
    ...
    def _run_audio(self, intention, duration):
        ...
        wave = self.audio.layer_frequencies([(f, 0.2) for f in frequencies], duration=duration)
...
def main():
    ...
    if choice == 1:
        blessing.start_blessing_session(intention="May all beings be peaceful", duration=60)
    elif choice == 2:
        blessing.start_blessing_session(intention="May healing energy flow to all who need it", duration=90)
    else:
        blessing.start_blessing_session(intention="May all beings find deep peace and stillness", duration=120)
