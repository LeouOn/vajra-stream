...
class WebSocketStabilityTester:
            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    await websocket.send(
                        json.dumps({"type": "ping", "connection_id": connection_id, "timestamp": time.time()})
                    )
    ...
    async def run_message_flood_test(
        self, connection_id: str = "flood_test", messages_per_second: int = 10, duration_seconds: int = 30
    ):
        ...
        try:
            async with websockets.connect(self.url) as websocket:
                logger.info("Flood test connected")
                ...
                for i in range(messages_per_second * duration_seconds):
                    message = {"type": "ping", "test_id": i, "timestamp": time.time(), "data": f"Test message {i}"}
