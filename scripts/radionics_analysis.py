...
class RadionicsAnalysisCLI:
    ...
    def cmd_analyze(self, args):
        ...
        result = self.analyzer.analyze_subject(args.subject, num_rates=args.num_rates, context=context)
    ...
    def cmd_gv(self, args):
        """Measure General Vitality."""
        print(f"\n{'=' * 70}")
        print("GENERAL VITALITY MEASUREMENT")
        print(f"{'=' * 70}")
        print(f"Subject: {args.subject}\n")

        if args.multiple:
            stats = self.gv_meter.measure_multiple(count=args.count, subject=args.subject)
    ...
    def cmd_signature(self, args):
        ...
        print(f"\n{'=' * 70}")
        print("SIGNATURE RATE GENERATION")
        print(f"{'=' * 70}")
        print(f"Subject: {args.subject}\n")
        ...
        for algo in algorithms:
            rate = self.sig_calculator.text_to_rate(
                args.subject, num_dials=args.dials, max_value=args.max_value, algorithm=algo.strip()
            )
    ...
    def cmd_broadcast(self, args):
        ...
        print(f"\n{'=' * 70}")
        print("RADIONICS BROADCASTING WITH FEEDBACK")
        print(f"{'=' * 70}")
        ...
        result = self.analyzer.broadcast_with_feedback(
            subject=args.subject, rate=rate, duration_seconds=args.duration, check_interval=args.interval
        )
    ...
    def cmd_balance(self, args):
        ...
        print(f"\n{'=' * 70}")
        print("BALANCING RATE GENERATION")
        print(f"{'=' * 70}")
        print(f"Subject: {args.subject}\n")

        balancing_rates = self.analyzer.find_balancing_rates(args.subject, num_rates=args.num_rates)
    ...
    def cmd_list_categories(self, args):
        ...
        print(f"\n{'=' * 70}")
        print("AVAILABLE RATE CATEGORIES")
        print(f"{'=' * 70}\n")
...
Examples:
  ...
  %(prog)s list-category chakra
    ...
    p_analyze.add_argument("--num-rates", type=int, default=10, help="Number of resonant rates to generate")
    p_analyze.add_argument("--with-astrology", action="store_true", help="Include astrological timing")
    p_analyze.add_argument("--save", action="store_true", help="Save results to file")

    # GV command
    p_gv = subparsers.add_parser("gv", help="Measure General Vitality")
    p_gv.add_argument("subject", help="Subject to measure")
    p_gv.add_argument("--multiple", action="store_true", help="Take multiple measurements")
    p_gv.add_argument("--count", type=int, default=10, help="Number of measurements (with --multiple)")
    p_gv.add_argument("--verbose", "-v", action="store_true", help="Show individual measurements")
    ...
    p_sig.add_argument(
        "--algorithm", default="mixed", choices=["hash", "gematria", "phonetic", "mixed"], help="Algorithm to use"
    )
    p_sig.add_argument("--dials", type=int, default=3, help="Number of rate values")
    p_sig.add_argument("--max-value", type=int, default=100, help="Maximum value per dial")
    ...
    p_broadcast.add_argument("--duration", type=int, default=300, help="Duration in seconds")
    p_broadcast.add_argument("--interval", type=int, default=60, help="GV check interval in seconds")
    p_broadcast.add_argument("--save", action="store_true", help="Save results to file")

    # Balance command
    p_balance = subparsers.add_parser("balance", help="Find balancing rates")
    p_balance.add_argument("subject", help="Subject to balance")
    p_balance.add_argument("--num-rates", type=int, default=5, help="Number of balancing rates")

    # Search command
    p_search = subparsers.add_parser("search", help="Search rate database")
    p_search.add_argument("query", help="Search term")
    p_search.add_argument("--exact", action="store_true", help="Exact match only")
    p_search.add_argument("--verbose", "-v", action="store_true", help="Show full details")
    ...
    p_list_cat.add_argument("--verbose", "-v", action="store_true", help="Show full details")
