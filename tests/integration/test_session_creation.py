    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("status") == "success" and "session_id" in result:
                ...
                start_response = requests.post(f"{base_url}/api/v1/sessions/{session_id}/start", timeout=10)
...
def test_incorrect_payload(base_url: str = "http://localhost:8001"):
    ...
    print("Sending incorrect payload:")
    ...
    try:
        ...
        if response.status_code == 422:
            print("Correctly rejected with 422 error (as expected)")
