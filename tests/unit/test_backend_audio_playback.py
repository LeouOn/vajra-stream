...
async def test_vajra_service():
    ...
    try:
        from backend.core.services.vajra_service import AudioConfig, vajra_service
        ...
        if audio_data is None:
            ...
            print("✅ Prayer bowl audio generated successfully")
...
async def test_api_endpoints():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post("http://localhost:8001/api/v1/audio/generate", json=generate_data) as response:
                if response.status == 200:
                    ...
                    async with session.post("http://localhost:8001/api/v1/audio/play", json=play_data) as play_response:
