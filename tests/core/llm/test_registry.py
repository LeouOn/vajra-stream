...
@pytest.mark.asyncio
async def test_registry_pick_skips_unhealthy_provider():
    """First selection must not send chat to a host that just failed health.

    A down LM Studio used to stay 'effectively healthy' for one blip and
    then sit in generate() until the Command Center 180s waiter fired.
    """
    registry = ProviderRegistry()
...
async def test_registry_pick_returns_none_when_all_unhealthy():
    ...
    assert await registry.pick_best(use_cache=False) is None
