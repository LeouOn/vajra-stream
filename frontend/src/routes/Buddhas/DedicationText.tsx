...
/** A single dedication-of-merit verse. */
...
interface DedicationTextProps {
  /** Optional callback fired when the user requests a new dedication. */
  onRefresh?: () => void;
}

function pickDedication(exclude: number = -1): Dedication {
...
export default function DedicationText({ onRefresh }: DedicationTextProps) {
  const [index, setIndex] = useState<number>(() => Math.floor(Math.random() * DEDICATIONS.length));
