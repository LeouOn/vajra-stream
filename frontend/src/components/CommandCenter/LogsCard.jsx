 ...
 * @component
 * @param {Object}                props
 * @param {'tools'|'debug'}       props.activeLogTab   - currently active tab key.
 * @param {Array}                 props.toolLogs       - tool-execution log entries.
 * @param {Object|null}           props.debugPayload   - LLM debug telemetry payload.
 * @param {React.RefObject}       props.logEndRef      - ref attached to the log scroll sentinel.
 * @param {(key: string) => void} props.onTabChange    - fired when the active tab changes.
 * @param {() => void}            props.onClearLogs    - fired when the CLEAR button is pressed.
 * @param {() => void}            props.onResetDebug   - fired when the RESET button is pressed.
 */
    ...
    <Card
      ...
      title={
        <Tabs
          ...
          onChange={(key) => { onTabChange?.(key); audioFeedback.playClick(); }}
