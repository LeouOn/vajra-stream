...
type RotationSpeed = { x: number; y: number; z: number };
...
interface RadionicsVisualizationSceneProps {
  attunedRate: number | string;
  isPlaying: boolean;
}

interface RadionicsVisualizationCanvasProps {
  attunedRate: number | string;
  isPlaying: boolean;
}

// Rotating crystal geometry component
const Crystal: React.FC<CrystalProps> = ({ position, scale, rotationSpeed, color, attunedRate: _attunedRate }) => {
...
const EnergyRing: React.FC<EnergyRingProps> = ({ radius, rotationSpeed, color, pulseSpeed }) => {
  const meshRef = useRef<THREE.Mesh>(null);
...
const ParticleField: React.FC<ParticleFieldProps> = ({ count, attunedRate }) => {
  ...
  useFrame(() => {
    if (particlesRef.current) {
      ...
      const rateFactor = parseFloat(String(attunedRate)) || 0;
...
const CentralCore: React.FC<CentralCoreProps> = ({ attunedRate }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  const materialRef = useRef<THREE.MeshStandardMaterial>(null);
  ...
  useFrame(() => {
    if (meshRef.current && materialRef.current) {
      ...
      const rate = parseFloat(String(attunedRate)) || 0;
...
function RadionicsVisualization({ attunedRate }: RadionicsVisualizationSceneProps) {
  const rate = parseFloat(String(attunedRate)) || 0;
...
const RadionicsVisualizationCanvas: React.FC<RadionicsVisualizationCanvasProps> = ({ attunedRate, isPlaying: _isPlaying }) => {
  return (
    <div className="w-full h-full">
      ...
      >
        <RadionicsVisualization attunedRate={attunedRate} isPlaying={_isPlaying} />
      </Canvas>
      ...
      <div className="absolute top-4 right-4 glassmorphism p-4 rounded-xl mystical-border max-w-xs">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className={`font-bold ${_isPlaying ? 'text-green-400 pulse-glow' : 'text-gray-400'}`}>
              {_isPlaying ? 'Active' : 'Idle'}
            </span>
          </div>
          <div className="flex justify-between">
...
export default RadionicsVisualizationCanvas;
