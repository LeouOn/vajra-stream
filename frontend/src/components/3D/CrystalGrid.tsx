 ...
 */
const CrystalGrid: React.FC<CrystalGridProps> = ({
  audioSpectrum,
  isPlaying,
  ...
  const crystalMaterials = useMemo<Record<CrystalType, CrystalMaterialProps>>(() => ({
    quartz: {
  ...
  const crystals = useMemo<CrystalObject[]>(() => {
    ...
    const crystalObjects: CrystalObject[] = [];

    config.crystals.forEach((crystal) => {
      // Calculate position
      ...
      if (crystal.x !== undefined && crystal.y !== undefined) {
        ...
        const angleRad = ((crystal.angle ?? 0) * Math.PI) / 180;
        x = Math.cos(angleRad) * (crystal.distance ?? 0) * 2;
        y = Math.sin(angleRad) * (crystal.distance ?? 0) * 2;
      }
  ...
  const energyField = useMemo<THREE.Mesh | null>(() => {
  ...
  const intentionObject = useMemo<THREE.Mesh | null>(() => {
  ...
  useFrame((state, delta) => {
    ...
    crystals.forEach((crystal, index) => {
      ...
      const crystalMaterial = crystal.mesh.material as THREE.MeshPhysicalMaterial;
      const glowMaterial = crystal.glow.material as THREE.MeshBasicMaterial;
      ...
      if (isPlaying && audioSpectrum.length > 0) {
        ...
        // Pulse glow
        if (crystal.glow) {
          glowMaterial.opacity = 0.2 + amplitude * 0.4;
          crystal.glow.scale.setScalar(1 + amplitude * 0.5);
        }

        // Increase emissive intensity
        crystalMaterial.emissiveIntensity = 0.3 + amplitude * 0.5;
        ...
        if (crystal.glow) {
          glowMaterial.opacity = 0.2 + Math.sin(time + index) * 0.1;
        }

        crystalMaterial.emissiveIntensity = 0.3 + Math.sin(time * 0.3 + index * 0.3) * 0.1;
    ...
    if (energyFieldRef.current) {
      ...
      const fieldMaterial = energyFieldRef.current.material as THREE.MeshBasicMaterial;

      // Pulse opacity based on audio
      if (isPlaying && audioSpectrum.length > 0) {
        const avgAmplitude = audioSpectrum.slice(0, 10).reduce((a, b) => a + b, 0) / 10;
        fieldMaterial.opacity = 0.3 + avgAmplitude * 0.4;
        energyFieldRef.current.scale.setScalar(1 + avgAmplitude * 0.2);
      } else {
        fieldMaterial.opacity = 0.3 + Math.sin(time * 0.5) * 0.1;
      }
    }
  });
