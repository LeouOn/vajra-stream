...
export default function Astrocartography({
  ...
  const createDefaultLines = (): React.ReactElement[] => {
    const defaultPlanets: PlanetaryLine[] = [
  ...
  const planetaryLineMeshes = useMemo((): React.ReactElement[] => {
    ...
    if (groupRef.current) {
      ...
      if (isPlaying && audioSpectrum.length > 0) {
        ...
        const birthMarkerGroup = groupRef.current.children.find((child) => {
          ...
          return first instanceof THREE.Mesh && first.geometry?.type === 'SphereGeometry';
        });
        if (birthMarkerGroup) {
  ...
  // Ambient particle positions (Float32Array of random values)
  const particlePositions = useMemo(() => {
    return new Float32Array(Array.from({ length: 3000 }, () =>
      (Math.random() - 0.5) * 15
    ));
  }, []);
