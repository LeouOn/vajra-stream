...
type SacredPattern = 'sri-yantra' | 'metatron' | 'seed-of-life' | 'tree-of-life' | 'chakra-mandala';
...
type PatternElement = THREE.Mesh | THREE.Line;
 ...
 */
const SacredMandala: React.FC<SacredMandalaProps> = ({
  audioSpectrum,
  isPlaying,
  frequency,
  pattern = 'sri-yantra',
  ...
  const elementRefs = useRef<Array<PatternElement | null>>([]);

  // Chakra color mappings
  const chakraColors = useMemo<Record<Chakra, ChakraColor>>(() => ({
  ...
  const createSriYantra = useMemo<PatternElement[]>(() => {
  ...
  const createMetatronsCube = useMemo<PatternElement[]>(() => {
    const elements: PatternElement[] = [];
    const baseColor = chakraColors[chakra] || chakraColors.heart;
  ...
  const createSeedOfLife = useMemo<PatternElement[]>(() => {
    const elements: PatternElement[] = [];
    const baseColor = chakraColors[chakra] || chakraColors.heart;
    const radius = 2;
  ...
  const createTreeOfLife = useMemo<PatternElement[]>(() => {
    const elements: PatternElement[] = [];
    const baseColor = chakraColors[chakra] || chakraColors.heart;

    // 10 Sephiroth positions
    const sephiroth: SephirahPosition[] = [
  ...
  // Select pattern
  const patternElements = useMemo<PatternElement[]>(() => {
    switch (pattern) {
      case 'sri-yantra':
  ...
  useFrame((state, delta) => {
    ...
    if (isPlaying && audioSpectrum.length > 0) {
      ...
      patternElements.forEach((element, index) => {
        const rawMaterial = element.material;
        ...
        if (material && 'opacity' in material) {
          ...
          mat.opacity = baseOpacity + amplitude * 0.3;
        }
      });
    } else {
    ...
    <group ref={groupRef}>
      {patternElements.map((element, index) => (
        <primitive
          key={index}
          object={element}
          ref={(el: PatternElement | null) => { elementRefs.current[index] = el; }}
        />
      ))}
    </group>
  );
