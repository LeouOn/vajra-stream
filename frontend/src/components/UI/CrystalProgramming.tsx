...
import type { CrystalLibraryEntry } from '../../stores/crystalStore';
...
const PROGRAMMING_STEPS: ProgrammingStep[] = [
...
type CrystalTab = 'program' | 'attune' | 'meditate';

interface CrystalProgrammingProps {
  className?: string;
}
...
const CrystalProgramming: React.FC<CrystalProgrammingProps> = ({ className = '' }) => {
  ...
  const [activeTab, setActiveTab] = useState<CrystalTab>('program');
  ...
  const handleStartProgramming = useCallback(() => {
    ...
    startProgramming(selectedCrystal?.id ? Number(selectedCrystal.id) || 0 : 0, intentionInput);
