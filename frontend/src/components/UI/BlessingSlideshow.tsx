...
interface SessionStats {
  photos_blessed?: number;
  total_mantras_repeated?: number;
  session_duration?: number;
  average_time_per_photo?: number;
  [key: string]: unknown;
}

interface Props {
  className?: string;
  onSessionChange?: ((sessionId: string | null, isActive: boolean) => void) | null;
}

const BlessingSlideshow: React.FC<Props> = ({ className = '', onSessionChange = null }) => {
  // Session state
  ...
  const [selectedMantra, setSelectedMantra] = useState<string>('chenrezig');
  const [customMantra, setCustomMantra] = useState<string>('');
  const [selectedIntentions, setSelectedIntentions] = useState<string[]>(['love', 'healing', 'peace']);
  const [dedication, setDedication] = useState<string>('May all beings benefit');
  const [repetitionsPerPhoto, setRepetitionsPerPhoto] = useState<number>(108);
  const [displayDuration, setDisplayDuration] = useState<number>(2000);
  const [loopMode, setLoopMode] = useState<boolean>(true);
  const [rngSessionId, setRngSessionId] = useState<string>('');
  ...
  const [mantraOpacity, setMantraOpacity] = useState<number>(0);
  const [intentionIndex, setIntentionIndex] = useState<number>(0);
  ...
  const mantraTimer = useRef<ReturnType<typeof setInterval> | null>(null);
  const intentionTimer = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  // Load current slide
  const loadCurrentSlide = async (sid?: string | null) => {
