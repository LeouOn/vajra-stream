...
interface ReplayTabProps {
  ...
  onRecompute?: (runId: number | string) => void;
}
...
interface SweepTabProps {
  ...
  onComplete?: (runId: number | string) => void;
}
...
const ReplayTab: React.FC<ReplayTabProps> = ({ onView, onRecompute }) => {
  ...
  const columns: TableColumnsType<RunRecord> = [
    ...
    { title: 'Completed', dataIndex: 'completed_tuples', key: 'completed_tuples', width: 100 },
    {
      ...
      key: 'status',
      width: 100,
      render: (s: string) => <Tag color={s === 'done' ? 'green' : s === 'error' ? 'red' : s === 'partial' ? 'orange' : 'blue'}>{s}</Tag>,
    },
    {
      title: 'Actions',
      key: 'actions',
...
const ResultsTab: React.FC<ResultsTabProps> = ({ currentRunId }) => {
...
const SweepTab: React.FC<SweepTabProps> = ({ setupState, onComplete }) => {
