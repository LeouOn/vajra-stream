...
interface CurrentStatus {
  current_population?: PopulationSummary;
...
interface SessionStats {
  ...
  total_photos_blessed?: number;
  total_rng_floating_needles?: number;
...
const AutomationControl: React.FC<Props> = ({ className = '' }) => {
  const [sessionId, setSessionId] = useState<string | null>(null);
  const [isActive, setIsActive] = useState<boolean>(false);
  const [isPaused, setIsPaused] = useState<boolean>(false);

  // Config
  const [durationPerPop, setDurationPerPop] = useState<number>(1800); // 30 min default
  const [transitionPause, setTransitionPause] = useState<number>(30);
  const [linkRng, setLinkRng] = useState<boolean>(true);
