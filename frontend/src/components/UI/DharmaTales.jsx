...
const DharmaTales = ({ className = '' }) => {
  ...
  const [availableTraditions, setAvailableTraditions] = useState([]);
  ...
  const deleteTale = useCallback((id) => {
    const saved = savedTales.filter(t => t.id !== id);
    setSavedTales(saved);
    localStorage.setItem('dharma-tales', JSON.stringify(saved));
  }, [savedTales]);
  
  const loadSavedTale = useCallback((savedTale) => {
