...
const WU_XING_COLORS = { Wood: '#10b981', Fire: '#ef4444', Earth: '#f59e0b', Metal: '#94a3b8', Water: '#3b82f6' };
const WU_XING_ICONS = { Wood: '🌳', Fire: '🔥', Earth: '⛰️', Metal: '⚔️', Water: '💧' };

export default function ChineseBaZi({ chineseData }) {
  if (!chineseData) return null;
  const bazi = chineseData.bazi || {};
  const lunar = chineseData.lunar_date || {};
  const shichen = chineseData.shichen || {};
  const zodiac = chineseData.zodiac_animal || '—';

  // Extract Wu Xing from BaZi pillars
  const countWuXing = () => {
    const counts = { Wood: 0, Fire: 0, Earth: 0, Metal: 0, Water: 0 };
    const charMap = { '木': 'Wood', '火': 'Fire', '土': 'Earth', '金': 'Metal', '水': 'Water' };
    Object.values(bazi).forEach(val => {
  ...
  const getWuXingFromPillar = (pillarStr) => {
