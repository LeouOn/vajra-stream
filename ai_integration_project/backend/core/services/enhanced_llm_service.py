    ...
    def _load_configuration(self):
        ...
        try:
            ...
            self.models["fallback"] = ModelConfig(
                provider=ModelProvider.OPENAI, model_name="gpt-4o-mini", api_key=os.getenv("OPENAI_API_KEY")
            )
...
You draw from Theravada, Mahayana, and Vajrayana traditions as appropriate.""",
            ...
            try:
                if config.provider == ModelProvider.Z_AI_GLM and config.api_key:
                    ...
                    self.lm_studio_client = AsyncOpenAI(
                        api_key=self.lm_studio_config.api_key, base_url=self.lm_studio_config.base_url
                    )
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.lm_studio_config.base_url}/v1/models", timeout=aiohttp.ClientTimeout(total=5)
                ) as response:
    ...
    async def _select_model(self, request: GenerationRequest) -> str:
        ...
        if request.preferred_provider:
            provider_models = [k for k, v in self.models.items() if v.provider == request.preferred_provider]
            if provider_models:
    ...
    async def _generate_z_ai_glm(self, request: GenerationRequest, config: ModelConfig) -> GenerationResponse:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": request.system_prompt or config.system_prompt or "You are a wise dharma teacher.",
                },
        ...
        async with self.z_ai_client.post(
            config.api_endpoint or "https://api.z-ai.com/v1/chat/completions", headers=headers, json=payload
        ) as response:
            if response.status == 200:
                data = await response.json()
    ...
    async def _generate_lm_studio(self, request: GenerationRequest, config: ModelConfig) -> GenerationResponse:
        ...
        response = await self.lm_studio_client.chat.completions.create(
            model=config.model_name, messages=messages, max_tokens=request.max_tokens, temperature=request.temperature
        )
    ...
    def _get_from_cache(self, request: GenerationRequest) -> GenerationResponse | None:
