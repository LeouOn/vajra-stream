...
class TTSRequest:
    ...
    provider_preference: TTSProvider | None = None
    speed: float = 1.0
    pitch: float = 1.0
    emotional_tone: str = "compassionate"
    output_format: str = "mp3"
    optimize_for_frequency: float | None = None  # Optimize for specific frequency
    use_cache: bool = True
...
class AdvancedTTSService:
    ...
    def _create_default_config(self, config_path: Path):
        ...
        default_config = {
            ...
            "pyttsx3": {"enabled": True, "rate": 150, "volume": 0.9, "voice_index": 0},
        }
    ...
    def _initialize_clients(self):
        ...
        try:
            # OpenAI TTS
            openai_api_key = next(
                (v.api_key for v in self.voices.values() if v.provider == TTSProvider.OPENAI_TTS and v.api_key), None
            )
            if openAI_api_key:
                self.openai_client = AsyncOpenAI(api_key=openai_api_key)

            # ElevenLabs
            elevenlabs_api_key = next(
                (v.api_key for v in self.voices.values() if v.provider == TTSProvider.ELEVENLABS and v.api_key), None
            )
    ...
    async def _select_voice(self, request: TTSRequest) -> str:
        ...
        if request.provider_preference:
            provider_voices = [k for k, v in self.voices.items() if v.provider == request.provider_preference]
            if provider_voices:
                return provider_voices[0]

        # Language-specific selection
        language_voices = [k for k, v in self.voices.items() if v.language == request.language]

        # Voice preference
        if request.voice_preference:
            preferred_voices = [k for k in language_voices if request.voice_preference in k.lower()]
            if preferred_voices:
                return preferred_voices[0]

        # Style-based selection
        style_voices = [k for k in language_voices if self.voices[k].style == request.emotional_tone]

        # Priority: ElevenLabs > OpenAI > Coqui > pyttsx3
        provider_priority = [TTSProvider.ELEVENLABS, TTSProvider.OPENAI_TTS, TTSProvider.COQUI_TTS, TTSProvider.PYTTSX3]

        for provider in provider_priority:
            provider_voices = [k for k in style_voices if self.voices[k].provider == provider]
            if provider_voices:
                return provider_voices[0]
    ...
    async def _synthesize_elevenlabs(self, request: TTSRequest, voice: VoiceConfig) -> TTSResponse:
        ...
        headers = {"xi-api-key": voice.api_key, "Content-Type": "application/json"}
        ...
        async with self.elevenlabs_client.post(
            "https://api.elevenlabs.io/v1/text-to-speech", headers=headers, json=payload
        ) as response:
            if response.status == 200:
                ...
                return TTSResponse(
                    ...
                    voice_used=voice.voice_id,
                    language=request.language,
                    metadata={"emotional_range": voice.emotional_range},
                )
            else:
    ...
    async def _synthesize_coqui(self, request: TTSRequest, voice: VoiceConfig) -> TTSResponse:
        ...
        if voice.model_path not in self.coqui_tts:
            ...
            try:
                self.coqui_tts[voice.model_path] = CoquiTTS(model_path=voice.model_path, progress_bar=False)
            except Exception as e:
    ...
    def _get_from_cache(self, request: TTSRequest) -> TTSResponse | None:
