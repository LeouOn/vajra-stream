...
class SpeechSynthesisRequest(BaseModel):
    ...
    voice_preference: str | None = Field(None, description="Preferred voice name")
    provider_preference: str | None = Field(None, description="Preferred provider (openai, elevenlabs, coqui)")
    ...
    optimize_for_frequency: float | None = Field(None, description="Optimize for specific frequency (432, 528, etc.)")
...
# Import services
from backend.core.services.advanced_tts_service import SacredLanguage, TTSProvider, TTSRequest, advanced_tts_service
...
async def create_guided_meditation(request: GuidedMeditationRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        from backend.core.services.enhanced_llm_service import GenerationRequest, enhanced_llm_service
...
async def get_voices_by_language(language: str):
    ...
    try:
        ...
        return {"status": "success", "language": language, "voice_count": len(voice_ids), "voices": voice_ids}
...
async def clear_tts_cache():
    ...
    try:
        ...
        return {"status": "success", "message": "TTS cache cleared successfully"}
